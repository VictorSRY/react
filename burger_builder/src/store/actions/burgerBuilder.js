...
export const removeIngredients=(ingredientName)=>{
    return{type: DELETE_INGREDIENT,ingredientName:ingredientName}
}
const setupdata=(ingredients) =>{
    return{type: SET_DATA,ingredients:ingredients}
}
const setError=(error)=>{
    return {type:ERROR,error:true}
}
export const getsetupdata=() => {
    return dispatch => oHttp.get("/ingredients.json").then(response=>{dispatch(setupdata(response.data))}).catch(error=>{console.log(error);dispatch(setError(error))})
}
